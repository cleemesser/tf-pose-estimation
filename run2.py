...
if __name__ == "__main__":
    ...
    image = common.read_imgfile(args.image, None, None)  # ndarray dtype=uint8,
    print(f"type(image):{type(image)}; image.dtype: {image.dtype}")
    if image is None:
    ...
    t = time.time()
    print(
        f"image.shape: {image.shape}, image.dtype: {image.dtype}, (w,h): {(w,h)}, {args.resize_out_ratio}"
    )
    # run run2.py --image ~/code/eegml/seizure-videos/myoclonic-atonic-vid02-dir/myoclonic-atonic-vid02.1.png
    # image.shape: (480, 582, 3), image.dtype: uint8, (w,h): (0, 0), 4.0
    humans = e.inference(
